...
class VoteBlock:
    ...
    def computeHash(self):
        ...
        blockString = str(self.index) + str(self.voteTo) + str(self.voteFrom) + \
            str(self.timestamp) + str(self.nonce) + str(self.previousBlockHash)
    ...
    def __str__(self):
        return '\nBlock#: %s\nVote To: %s\nVote From: %s\nTime: %s\nNonce: %s\nHash: %s\nPrevious Hash: %s' % (self.index, self.voteTo, self.voteFrom, self.timestamp, self.nonce, self.blockHash, self.previousBlockHash)
